...
$apps = $config['portal']['apps'] ?? [
    ...
    ['name' => 'Agenda',        'url' => 'https://calendar.google.com','emoji' => '📅'],
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        body::before {
            ...
            inset: 0;
            background:
                radial-gradient(ellipse 80% 60% at 10% 0%,   rgba(52,84,209,.35) 0%, transparent 60%),
        ...
        /* Cards apps */
        .app-card { transition: transform .18s ease, background .18s ease, box-shadow .18s ease; }
        .app-card:hover  { transform: translateY(-4px) scale(1.04); box-shadow: 0 12px 30px rgba(0,0,0,.4); background: rgba(255,255,255,.12); }
        .app-card:active { transform: translateY(-1px) scale(1.01); }

        /* Délais d'animation par colonne */
        ...
        .app-card:nth-child(7)  { animation-delay: .35s }
        .app-card:nth-child(8)  { animation-delay: .40s }
        .app-card:nth-child(9)  { animation-delay: .45s }
        .app-card:nth-child(10) { animation-delay: .50s }
        .app-card:nth-child(11) { animation-delay: .55s }
        .app-card:nth-child(12) { animation-delay: .60s }

        /* Horloge */
        #clock { font-variant-numeric: tabular-nums; letter-spacing: .02em; }
    </style>
</head>
<body class="min-h-screen text-slate-100 relative" style="background: #020617;">

    <?php include __DIR__ . '/_nav.php'; ?>

    <main class="relative z-10 container mx-auto px-4 py-8 max-w-6xl space-y-8">

        <!-- ── Hero : profil + horloge ──────────────────────────────────── -->
        <section class="glass rounded-3xl p-6 sm:p-8 animate-fade-in">
            <div class="flex flex-col sm:flex-row items-center sm:items-start justify-between gap-6">
                ...
                <div class="flex items-center gap-5">
                    ...
                    <img src="<?= htmlspecialchars($user['picture']) ?>"
                         alt="Photo de profil"
                         class="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl border-2 border-white/20 shadow-xl flex-shrink-0">
                    <?php endif; ?>
                    <div>
                        ...
                        <p class="text-gray-400 text-sm mt-0.5"><?= htmlspecialchars($user['email']) ?></p>
                    </div>
                </div>

                <!-- Horloge + date -->
                <div class="text-center sm:text-right flex-shrink-0">
                    <p id="clock" class="text-4xl sm:text-5xl font-bold text-white tracking-tight">--:--:--</p>
                    <p id="date-display" class="text-gray-400 text-sm mt-1 capitalize">--</p>
                </div>
            </div>
            ...
            <form action="https://www.google.com/search" method="get"
                ...
                <input type="text" name="q" placeholder="Rechercher sur Google…"
                       autocomplete="off"
                       class="flex-1 px-4 py-3 rounded-xl bg-white/10 border border-white/15 text-white placeholder-gray-400
                              focus:outline-none focus:ring-2 focus:ring-brand-cyan transition text-sm">
                <button type="submit"
                        class="px-5 py-3 bg-brand-indigo hover:bg-blue-700 text-white font-semibold rounded-xl transition shadow text-sm whitespace-nowrap">
                    Rechercher
                </button>
            </form>
        </section>
        ...
        <section class="animate-slide-up" style="animation-delay:.1s">
            <h2 class="text-base font-semibold text-gray-400 uppercase tracking-widest mb-3">📌 À la une</h2>
            <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div class="glass rounded-2xl p-4 shadow"
                     style="border-left: 3px solid <?= $accentColor ?>;">
                    <div class="flex items-start gap-3">
                        <span class="text-xl select-none mt-0.5"><?= $annEmoji ?></span>
                        <div class="min-w-0">
                            <?php if ($annTitle): ?>
                            <p class="font-semibold text-white text-sm leading-snug mb-1"><?= $annTitle ?></p>
                            <?php endif; ?>
                            <p class="text-gray-300 text-sm leading-relaxed"><?= nl2br($annContent) ?></p>
                            <?php if ($annDate): ?>
                            <p class="text-gray-500 text-xs mt-2"><?= $annDate ?></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
        ...
        <section class="animate-slide-up" style="animation-delay:.2s">
            <h2 class="text-base font-semibold text-gray-400 uppercase tracking-widest mb-3">Applications</h2>
            <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3">
                ...
                ?>
                <a href="<?= $appUrl ?>"
                   class="app-card glass animate-fade-in rounded-2xl p-4 flex flex-col items-center gap-2 cursor-pointer">
                    <span class="text-3xl select-none"><?= $appEmoji ?></span>
                    <span class="text-xs font-semibold text-gray-300 text-center leading-tight"><?= $appName ?></span>
                </a>
                <?php endforeach; ?>
            </div>
        </section>
    ...
    <script>
        ...
        function tick() {
            ...
            dateEl.textContent = `${jour} ${day} ${mo} ${year}`;
        }
